...
class MeanScaleUniformBins(ChronosTokenizer):
    ...
    def _append_eos_token(
        ...
        fill_value=self.config.eos_token_id, device = self.device)
        token_ids = torch.concat((token_ids, eos_tokens), dim=1)
        eos_mask = torch.full((batch_size, 1), fill_value=True, device=self.device)
    ...
    def output_transform(
        ...
        scale_unsqueezed = scale.unsqueeze(-1).unsqueeze(-1).to(self.device)
        indices = torch.clamp(
    ...
    def predict(
        ...
        while remaining > 0:
            ...
            prediction = self.tokenizer.output_transform(
                samples.to(self.model.device), scale
            )
            ...
            context_tensor = context_tensor.to(device=self.model.device)
