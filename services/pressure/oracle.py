...
class PressureOracle:
    ...
    def detect_market_regime(
        self, price_history: np.ndarray, current_idx: int
    ) -> MarketRegime:
        """
    ...
    def compute_pressure_labels(
        ...
        normalized_strength = move_magnitude / (
            local_vol * np.sqrt(self.lookahead_candles) + 1e-9
        )
...
class TemporalDatasetSplitter:
    ...
    def split_temporal(
        ...
        logger.info(
            f"Temporal split: Train={len(train_indices)}, Val={len(val_indices)}, Test={len(test_indices)}"
        )
        logger.info(
            f"Train: [0, {train_end}), Val: [{val_start}, {val_end}), Test: [{test_start}, {data_length})"
        )
    ...
    def walk_forward_split(
        data_length: int, train_size: int, val_size: int, step: int = None
...
if __name__ == "__main__":
    ...
    for i in list(train_idx)[:5]:
        ...
        # Generate label
        label = oracle.compute_pressure_labels(snapshot, future, prices, i)
