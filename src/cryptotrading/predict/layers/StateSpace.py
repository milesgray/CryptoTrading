...
class StateSpaceLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        A_input = torch.sigmoid(self.A_proj(x_seq))
        ...
        for t in range(seq_len):
            ...
            h = A[:, t] * h + self.B(x_seq[:, t])
            
            # Generate output
            y = self.C(h) + self.D + x_seq[:, t]  # Skip connection
            outputs.append(y.unsqueeze(1))
        
        # Combine outputs
        output = torch.cat(outputs, dim=1)
        res = self.projection(output)
        return res.squeeze(1) if is_2d else res