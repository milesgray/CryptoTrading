...
def spawner(x, labels, sigma=0.05, verbose=0):
    ...
    orig_steps = np.arange(x.shape[1])
    label_max = np.argmax(labels, axis=1) if labels.ndim > 1 else labels
    ...
    for i, pat in enumerate(x):
        ...
        choices = np.where(label_max[choices] == label_max[i])[0]
        if choices.size > 0:     
...
def wdba(x, labels, batch_size=6, slope_constraint="symmetric", use_window=True, verbose=0):
    ...
    for i in range(ret.shape[0]):
        ...
        choices = np.where(label_max == label_max[i])[0]
        if choices.size > 0:        
...
def discriminative_guided_warp(x, labels, batch_size=6, slope_constraint="symmetric", use_window=True, dtw_type="normal", use_variable_slice=True, verbose=0):
    ...
    for i, pat in enumerate(x):
        ...
        # remove ones of different classes
        positive = np.where(label_max[choices] == label_max[i])[0]
        negative = np.where(label_max[choices] != label_max[i])[0]
...
def rnd_augment(rounds=1):    
    import random
    from . import augment as aug        
    augmentations = [
        lambda x: aug.jitter(x),
        lambda x: aug.scaling(x),
        lambda x: aug.rotation(x),
        lambda x: aug.magnitude_warp(x),
        lambda x: aug.time_warp(x),
        lambda x: aug.window_slice(x),
    ]
    indexes = [n for n in range(len(augmentations))]
    sequence = random.sample(indexes, k=min(rounds, len(indexes)))    
    return sequence, augmentations
    
def sequence_augment(x, sequence, augmentations=[]):
    """
    Apply a sequence of augmentations to the input data.
    
    Args:
        x: Input data
        sequence: List of indices indicating which augmentations to apply
